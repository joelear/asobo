...
class AsoboSupabaseUser extends BaseAuthUser {
  AsoboSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> asoboSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = AsoboSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
