...
Future<List<CalendarMonthStruct>> createCalendar() async {
  // Add your function code here!

  List<CalendarMonthStruct> calendar = [];
  ...
  for (int year = startDate.year; year <= endDate.year; year++) {
    for (int month = 1; month <= 12; month++) {
      ...
      // Determine the start and end dates for the complete weeks covering the month
      ...
      while (currentDay.isBefore(endOfWeek) ||
        ...
        // Create a week of 7 days (Monday to Sunday)
        for (int i = 0; i < 7; i++) {
          days.add(CalendarDayStruct(date: currentDay));
      ...
      calendar.add(CalendarMonthStruct(
        year: year,
        month: month,
        weeks: weeks,
      ));
